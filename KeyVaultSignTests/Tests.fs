...
type TestClass () =
    ...
    member this.``KeyVault sign async test``() =
        ...
        task {
            let! signature = KeyVault.signAsync this.keyvault this.certificateName "Here's a message"
            // signature |> printfn "%A"
            Assert.IsNotNull(signature);
            Assert.AreEqual(256, signature.Signature.Length)
            Assert.IsTrue (signature.KeyId.Contains this.keyvault)
    ...
    member this.``KeyVault sign and verifyPublic test``() =
        ...
        let signBytes = signature |> Convert.FromBase64String
        ...
        let res = KeyVault.verifyPublic publicKeyXml signBytes msg
        // res |> printfn "%A"
        Assert.IsNotNull(res);
        Assert.IsTrue res
